...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/12/21 16:56:05 by bliu              #+#    #+#             */
/*   Updated: 2025/12/21 17:01:54 by bliu             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	quad_hit(t_ray *ray, t_interval ray_t, t_object obj, t_hit_record *record)
{
	...
	if (!interval_contains(&ray_t, t))
		return (0);
	pl_hitp_vec = vec3_sub(ray_at(ray, t), obj.geo.quad.q);
	alpha = vec3_dot(obj.geo.quad.w, vec3_cross(pl_hitp_vec, obj.geo.quad.v));
	beta = vec3_dot(obj.geo.quad.w, vec3_cross(obj.geo.quad.u, pl_hitp_vec));
